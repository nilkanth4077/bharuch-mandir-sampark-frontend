...
function AddSupervisorModal({ modal, setModal }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.phone_number) errs.phone_number = "સુપરવાઇજરનો ફોન નંબર લખો";
        ...
        <ModalBody>
          ...
          <FormControl fullWidth variant="outlined" margin="normal" size="small">
            ...
            >
              ...
              {!booksLoading && !booksError && myBooks.length === 0 && (
                <MenuItem disabled>No assigned posts</MenuItem>
              )}
              {myBooks.map((b) => (
          ...
          <FormControl fullWidth variant="outlined" margin="normal" size="small">
            ...
            >
              ...
              {booksError && <MenuItem disabled>{booksError}</MenuItem>}
              {!booksLoading && !booksError && myBooks.length === 0 && (
                <MenuItem disabled>No assigned mandal</MenuItem>
              )}
              {myBooks.map((b) => (
                <MenuItem key={b.book_no} value={String(b.book_no)}>
                  {bookLabel(b)}
